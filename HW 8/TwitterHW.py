...
def get_tweets():
    if "umsi" in CACHE_DICTION: ## Checking if umsi is already in cache; if so return value
        return CACHE_DICTION["umsi"]
    else: ## Filling in cache with publictweets.
        umsitweets = api.search("umsi")
        try:
            CACHE_DICTION["umsi"] = umsitweets
            ...
            print("Something broke and I am very sad")
            return None
...
umsi_dict = get_tweets()
umsi_tweets = umsi_dict["statuses"]
#print(umsi_tweets["statuses"])
...
for prop in umsi_tweets:
    info = (prop["id"], prop["user"]["screen_name"], prop["created_at"], prop["text"], prop["retweet_count"])
    cur.execute('INSERT INTO Tweets (tweet_id, author, time_posted, tweet_text, retweets) VALUES (?, ?, ?, ?, ?)', info)
